...
public class EstoqueSpecification {
    ...
    public static Specification<Estoque> filtrar(EstoqueFiltro filtro, Long empresaId) {

        return (root, query, cb) -> {

            List<Predicate> predicates = new ArrayList<>();

            predicates.add(cb.equal(root.get("loja").get("empresa").get("id"), empresaId));
            if (filtro == null) {
            ...
            if (filtro.nomeProduto() != null && !filtro.nomeProduto().isBlank()) {
                predicates.add(cb.like(cb.lower(root.get("produtoVariacao").get("produto").get("name")), "%" + filtro.nomeProduto().trim().toLowerCase(Locale.ROOT) + "%"));
            }

            if (filtro.nomeVariacao() != null && !filtro.nomeVariacao().isBlank()) {
                predicates.add(cb.like(cb.lower(root.get("produtoVariacao").get("nomeVariacao")), "%" + filtro.nomeVariacao().trim().toLowerCase(Locale.ROOT) + "%"));
            }

            if (filtro.categoriaId() != null) {
                predicates.add(cb.equal(root.get("produtoVariacao").get("produto").get("categoria").get("id"), filtro.categoriaId()));
            }

            if (filtro.nomeCategoria() != null && !filtro.nomeCategoria().isBlank()) {
                predicates.add(cb.like(cb.lower(root.get("produtoVariacao").get("produto").get("categoria").get("nomeCategoria")), "%" + filtro.nomeCategoria().trim().toLowerCase(Locale.ROOT) + "%"));
            }
