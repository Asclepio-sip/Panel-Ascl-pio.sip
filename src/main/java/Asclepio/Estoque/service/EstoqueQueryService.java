...
public class EstoqueQueryService {
    ...
    public Page<ListaDeEstoqueDasLojasResponse> listarTodos(EstoqueFiltro filtro, Pageable pageable) {
        return estoqueRepository
                .findAll(EstoqueSpecification.filtrar(filtro, empresaContext.getEmpresaId()), pageable)
                .map(estoque -> {
                    // Busca local no banco via ProdutoVariacaoService
                    ProdutoVariacaoResponse variacao = produtoVariacaoService.buscarPorIdDTO(estoque.getVariacaoId());
    ...
    public Estoque buscarPorId(Long id, Long empresaId) {
        return estoqueRepository.findByIdAndLoja_Empresa_Id(id, empresaId)
    ...
    public Estoque buscarPorLojaEVariacao(Long lojaId, Long variacaoId, Long empresaId) {
        return estoqueRepository.findByLoja_IdAndLoja_Empresa_IdAndVariacaoId(lojaId, empresaId, variacaoId)
                .orElseThrow(() -> new ResourceNotFoundException("Estoque não encontrado"));
    }
}