...
public interface LojaApi {
    ...
    @Operation(summary = "Criar loja", description = "Cria uma nova loja/farmácia no sistema. Nome, CEP, telefone e tipo de atendimento são obrigatórios.")
    ...
    @Operation(summary = "Atualizar loja", description = "Atualiza os dados principais de uma loja existente pelo ID.")
    ResponseEntity<LojaResponse> atualizar(

            @Parameter(description = "ID da loja", example = "1") @PathVariable Long id,

            @RequestBody CreateLojaRequest request);

    @DeleteMapping("/{id}")
    @PreAuthorize("hasAuthority('ExcluirLoja')")
    @Operation(summary = "Excluir loja", description = "Remove uma loja pelo ID. Caso não exista, retorna 404 pelo tratamento global de erros.")
