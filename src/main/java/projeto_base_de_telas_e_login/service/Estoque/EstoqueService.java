...
public class EstoqueService {
    ...
    public void criar(EstoqueAddDto dto) {
        ...
        if (dto.lojaID() == null && dto.nomeLoja() == null) {
            throw new RuntimeException(
                    "Informe lojaId ou nomeLoja"
            );
        }

        if (dto.produtoId() == null && dto.nomeProduto() == null) {
            throw new RuntimeException(
                    "Informe produtoId ou nomeProduto"
            );
        }
    ...
    public List<Estoque> buscarPorNomeLoja(String nomeLoja) {
        return estoqueRepository
                .findByLoja_NomeContainingIgnoreCase(nomeLoja);
    }

    @Transactional(readOnly = true)
    public List<Estoque> buscarPorNomeProduto(String nomeProduto) {
        return estoqueRepository
                .findByProduto_NameContainingIgnoreCase(nomeProduto);
    }

    @Transactional(readOnly = true)
    public List<Estoque> filtrar(
            Long lojaId,
            String nomeLoja,
            Boolean semEstoque
    ) {
        return estoqueRepository.filtrar(
                lojaId,
                nomeLoja,
                semEstoque
        );
